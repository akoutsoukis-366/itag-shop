...
return (
<div className="flex items-center justify-between rounded border p-4" aria-busy={pending}>
<div>
<div className="font-medium">{row.title}</div>
<div className="text-sm text-gray-600">
Unit €{(row.unitCents / 100).toFixed(2)} - Qty {qty} - Line €{(lineCents / 100).toFixed(2)}
</div>
</div>

  <div className="flex items-center gap-2">
    <button className="rounded border px-2 py-1" onClick={() => applyQuantity(qty - 1)} disabled={pending}>
      −
    </button>
    <button className="rounded border px-2 py-1" onClick={() => applyQuantity(qty + 1)} disabled={pending}>
      +
    </button>
    <button className="rounded border px-2 py-1 text-red-600" onClick={remove} disabled={pending}>
      Remove
    </button>
  </div>
</div>
